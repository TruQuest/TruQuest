    ...
    protected Repository(DbContext dbContext)
    {
        ...
        if (DbConnectionProvider.TryGet(out DbConnection? dbConn))
        {
            ...
            if (dbConnToReplace != dbConn!)
            {
                ...
                // earlier than all the rest (e.g. in a thataway middleware). In such a case, since
                // thataway handlers get resolved "just in time", when the rest get created later,
