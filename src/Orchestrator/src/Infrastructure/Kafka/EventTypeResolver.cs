...
internal class EventTypeResolver : IMessageTypeResolver
{
    private const string _eventTypeHeaderName = "Type";
    ...
    public ValueTask<Type> OnConsumeAsync(IMessageContext context)
    {
        ...
        {
            var messageTypeBytes = context.Headers[_eventTypeHeaderName];
        ...
        if (type != null) return ValueTask.FromResult(type);

        throw new InvalidOperationException();
    }

    public ValueTask OnProduceAsync(IMessageContext context) => throw new NotImplementedException();
}
